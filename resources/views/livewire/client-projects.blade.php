<div class="p-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <a href="{{ route('learning.project', $project->id) }}" class="group block rounded-xl border border-gray-200 bg-white p-6 shadow-sm
            ...
            <div class="flex items-start justify-between mb-3">
                <h2 class="text-lg font-semibold text-gray-800 group-hover:text-blue-600">
                    {{ $project->name }}
                    @if($project->new_learnings_count > 0)
                    <span class="rounded-full text-white bg-rose-500 px-2">
                        {{ $project->new_learnings_count }}
                    </span>
                    @endif
                </h2>

                <!-- Arrow icon -->
                <svg class="w-5 h-5 text-gray-400 group-hover:text-blue-600 transition" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
